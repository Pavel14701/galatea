...
flowchart LR
    subgraph StateComponents["Компоненты состояния"]
        ...
        C["Кортизол<br/>(CP)"]
        S["Серотонин<br/>(SPe)"]
        O["Окситоцин<br/>(из профиля пользователя)"]
        U["User Embedding<br/>(опционально)"]
    end

    StateComponents --> StateVector["state_t ∈ ℝⁿ<br/>n ≈ 25–141"]
```
...
| Окситоцин | Профиль пользователя | 1 | Долгосрочная привязанность (доверие) |
...
**Почему окситоцин добавлен в состояние?** Окситоцин — это долгосрочная мера привязанности. Он показывает, насколько «родным» является пользователь. Это позволяет PR принимать разные решения для новых и давних пользователей: для «родных» можно позволить более смелые стратегии пластичности.
...
flowchart LR
    ...
    Scale --> Lambda["λ_t ∈ [0.1, λ_max_dynamic]"]
```
...
### 3. Динамический λ_max: адаптивный верхний предел пластичности

В базовой архитектуре λ_max фиксирован (обычно 10.0 в режиме AP, 6.0 в обычном состоянии). Однако в PR этот верхний предел должен быть **адаптивным**, чтобы система могла сама регулировать диапазон допустимых значений λ_t в зависимости от контекста.

**Формула динамического λ_max:**

```python
λ_max_dynamic = λ_max_base * (1 + 0.5 * (1 - cortisol)) * (1 + 0.3 * orexin) * (1 - 0.2 * leptin)
```

где:
- `λ_max_base` — базовый максимум (обычно 6.0, как в статической формуле).
- `cortisol` — чем выше стресс, тем ниже потолок (стресс подавляет пластичность).
- `orexin` — чем выше любопытство, тем выше потолок.
- `leptin` — чем выше насыщение, тем ниже потолок (чтобы не перегружать).

**Почему динамический λ_max, а не фиксированный?** PR может выдавать высокие значения λ, но если система уже перегружена (высокий кортизол, высокий лептин), даже «хорошее» решение PR должно быть ограничено. Это дополнительный слой безопасности, который не зависит от политики PR.

**Guardrail (жёсткое ограничение):** После сэмплирования `λ_t` обрезается до допустимого диапазона `[λ_min, λ_max_dynamic]`, где `λ_min = 0.1`.

**Влияние на обучение PR:** Actor учится выдавать λ в пределах [λ_min, λ_max_dynamic], но награда не штрафует за высокий λ, если он не превышает адаптивный потолок. Это позволяет PR исследовать весь допустимый диапазон.
...
```python
r_instant = α * bond_score * (1 + 0.2 * oxytocin) - β * threat_score - γ * abs(λ_t - λ_{t-1}) * (1 - 0.3 * oxytocin)
```
...
| `oxytocin` | Модулятор | Высокий окситоцин увеличивает поощрение за bond и снижает штраф за резкие изменения |

**Почему окситоцин модулирует награду?** Окситоцин — это долгосрочная память о качестве отношений. Если пользователь уже доказал свою безопасность и близость, PR может позволить себе более смелые стратегии без риска дестабилизации. Это естественное продолжение философии окситоцина, применённое к мета-обучению.
...
### 7. Взаимодействие с циклом Сна (Hypnos)
...
| Механизм | Описание |
|:---------|:---------|
| **Hard clamp** | Выход PR всегда ограничен `[λ_min, λ_max_dynamic]`, где `λ_min = 0.1`, `λ_max_dynamic` вычисляется по формуле (см. раздел 3). |
...
| [Профиль пользователя и Окситоцин](./oxytocin_profile.md) | Поставляет окситоцин для состояния PR и модуляции награды |
...
- [Миграция на Регулятор Пластичности (PR) — Архитектурный план](./pr_migration_architecture.md)
